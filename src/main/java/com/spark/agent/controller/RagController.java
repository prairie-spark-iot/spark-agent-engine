...
public class RagController {
    ...
    record IngestRequest(@NotBlank String title, @NotBlank String content, Short docType,
                         String deviceModel, Long productId, String source) {}

    record SearchRequest(String query, String deviceModel, int topK) {}

    @PostMapping("/ingest")
    public R<Map<String, Integer>> ingest(@RequestBody @Valid IngestRequest req) {
    ...
    public R<List<VectorStoreRepository.SearchResult>> search(@RequestBody SearchRequest req) {
        if (req.query() == null || req.query().isBlank()) {
            return R.fail(400, "query must not be empty");
        }
